...
| _Everything Else that isn't GitHub related or legal_ | Just some little fun                                                                                                                 |
...
Please do not commit large binary assets directly without checking size first.

- Keep reference images small and tightly cropped (for example plugin icon snippets under `resources/img-refs`).
- Before committing audio/media, verify file sizes and avoid adding very large files by accident.
- If a contribution needs large media assets, discuss it in the PR first so maintainers can decide whether to use Git LFS or another distribution approach.

### Building with AHK

_Presuming the location of your AutoHotKey install is `C:\Program Files\AutoHotkey\`and you have git installed_
...
- Run the AutoHotKey compiler `"C:\Program Files\AutoHotkey\Compiler\Ahk2Exe.exe" /in LiveEnhancementSuite.ahk /out "Live Enhancement Suite.exe"`
  ...
  **_Sidenote: If you are trying to debug the LiveEnhancementSuite script, you may run it as a normal AutoHotKey script, building is generally reserved for releases_**
